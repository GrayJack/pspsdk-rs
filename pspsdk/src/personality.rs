    ...
    cfg_select! {
        all(target_abi = "pauthtest", target_arch = "aarch64") => {
            ...
            unsafe {
                ...
                // `pacib` corresponds to `ptrauth_key_process_dependent_code` in
                // <ptrauth.h>.
                core::arch::asm!(
                    "pacib {addr}, {sp}",
                    addr = inout(reg) addr,
...
unsafe extern "C" fn rust_eh_personality_impl(
    version: c_int, actions: uw::_Unwind_Action, _exception_class: uw::_Unwind_Exception_Class,
...
///   _UA_SEARCH_PHASE flag as described below, first for the current PC and register state, and
///   then unwinding a frame to a new PC at each step..."
/// - "If the search phase reports success, the framework restarts in the cleanup phase. Again, it
///   repeatedly calls the personality routine, with the _UA_CLEANUP_PHASE flag as described below,
///   first for the current PC and register state, and then unwinding a frame to a new PC at each
///   step..."i
///
...
unsafe extern "C" fn rust_eh_personality(
    version: c_int, actions: uw::_Unwind_Action, exception_class: uw::_Unwind_Exception_Class,
    exception_object: *mut uw::_Unwind_Exception, context: *mut uw::_Unwind_Context,
) -> uw::_Unwind_Reason_Code {
...
#[lang = "eh_personality"]
#[cfg(all(not(feature = "std"), not(panic = "unwind")))]
unsafe extern "C" fn rust_eh_personality() {
