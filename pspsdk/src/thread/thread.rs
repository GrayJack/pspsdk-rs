...
/// two ways:
///
/// * By spawning a new thread, e.g., using the [`thread::spawn`] function, and calling [`thread`]
///   on the [`JoinHandle`].
...
impl Thread {
    ...
    pub(crate) fn set_os_id_to_current(&self) {
        if let Some(os_id) = imp::current_os_id()
            && self.inner.os_id.set(os_id).is_err()
        {
            crate::rtabort!("thread OS id already set");
        }
    }
    ...
    ///
    /// let handler = builder
    ///     .spawn(|| assert_eq!(thread::current().name(), Some("foo")))
    ///     .unwrap();
    ///
    /// handler.join().unwrap();
    /// ```
    ...
    pub unsafe fn from_raw(ptr: *const ()) -> Thread {
        ...
        unsafe {
            Thread {
                inner: Pin::new_unchecked(Arc::from_raw_in(ptr as *const Inner, System)),
            }
        }
    }
