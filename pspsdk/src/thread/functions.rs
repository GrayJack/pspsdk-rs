...
///
/// * The process aborts.
/// * The process does not abort, and [`join`] will return a `Result::Err` containing an opaque
///   type.
///
...
/// ```
#[deprecated(since = "1.6.0", note = "replaced by `pspsdk::thread::sleep`")]
...
impl Drop for PanicGuard {
    fn drop(&mut self) {
        crate::rtabort!("an irrecoverable error occurred while synchronizing threads")
    }
}
...
///   already. Because the token can be held by a thread even if it is currently not parked,
///   [`unpark`] followed by [`park`] will result in the second call returning immediately. However,
///   note that to rely on this guarantee, you need to make sure that your `unpark` happens after
///   all `park` that may be done by other data structures!
///
...
///   CPUs, because it reports only the logical CPUs in one processor group. Before Windows 11 and
///   Windows Server 2022, a process was by default confined to a single processor group, so this
///   count reflected the CPUs it could use without explicitly opting into other groups. Starting
///   with Windows 11 and Windows Server 2022, a process and its threads have affinities that by
///   default span all processor groups, so on systems with more than 64 logical CPUs this may
///   report fewer CPUs than are available to the program.
...
#[doc(alias = "num_cpus")] // Alias for a popular ecosystem crate which provides similar
                           // functionality.
