...
//! thread-safe are easily shared between threads using the
...
///
/// 1. propagate the panic with [`pspsdk::panic::resume_unwind`]
/// 2. or in case the thread is intended to be a subsystem boundary that is supposed to isolate
///    system-level failures, match on the `Err` variant and handle the panic in an appropriate way
///
